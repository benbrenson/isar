...
do_setup_buildchroot() {
    ...
    if [ -x "${CROSS_BUILDCHROOT_DIR}/sbin/start-stop-daemon" ]; then
        ...
        sudo tee ${CROSS_BUILDCHROOT_DIR}/sbin/start-stop-daemon > /dev/null  <<-__EOF__
			#!/bin/sh
			echo
    ...
    if [ -x "${CROSS_BUILDCHROOT_DIR}/sbin/initctl" ]; then
        ...
        sudo tee ${CROSS_BUILDCHROOT_DIR}/sbin/initctl > /dev/null <<-__EOF__
			#!/bin/sh
			echo
    ...
    if [ -x "${CROSS_BUILDCHROOT_DIR}/sbin/init" -a ! -f "${CROSS_BUILDCHROOT_DIR}/usr/sbin/policy-rc.d" ]; then
        ...
        sudo tee ${CROSS_BUILDCHROOT_DIR}/usr/sbin/policy-rc.d > /dev/null <<-__EOF__
			#!/bin/sh
